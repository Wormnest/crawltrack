...
class awPHPFontDriver extends awFontDriver
{
    ...
    private function gdString(awGDDriver $driver, awText $text, awPoint $point, $width = NULL)
    {
        ...
        foreach ($lines as $i => $line) {
            ...
            $function(
                $driver->resource, $font->font, $driver->x + $point->x + $addX, $driver->y + $point->y + $addY + $addAngle, $line, $rgb
            );
        }
    }
...
class awFileFontDriver extends awFontDriver
{
    ...
    private function gdString(awGDDriver $driver, awText $text, awPoint $point, $width = NULL)
    {
        ...
        imagettftext(
            $driver->resource, $font->getSize(), $text->getAngle(), $driver->x + $point->x + $textWidth * sin($text->getAngle() / 180 * M_PI), $driver->y + $point->y + $textHeight, $rgb, $filePath, $textString
        );
    }
