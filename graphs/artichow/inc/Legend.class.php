...
//			$font = $text->getFont();
        ...
        // Draw legend shadow
        $this->shadow->draw(
            $driver, $p, $p->move($width, $height), awShadow::OUT
        );
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $driver->string(
                $texts[$i], $p->move(
                    $this->padding[0] + $previousColumns + 10 + 5 + $this->textMargin->left, $this->padding[2] + $row * ($heightMax + $this->space) + $heightMax / 2 - $heights[$i] / 2
                )
            );
                ...
                case awLegend::MARKONLY :
                        ...
                        if ($color instanceof awColor and $type !== awLegend::MARKONLY) {
                            ...
                            $driver->line(
                                $color, new awLine(
                                    ...
                                    $p->move(
                                        $x + 10, $y + $component->getLegendLineThickness() / 2
                                    ),
                    ...
                    if ($type === awLegend::MARK or $type === awLegend::MARKONLY) {
                        ...
                        if ($mark !== NULL) {
                            $mark->draw(
                                $driver, $p->move(
                                    $x + 5.5, $y + $component->getLegendLineThickness() / 2
                                )
                            );
                        }
                ...
                case awLegend::BACKGROUND :
                    ...
                    if ($background !== NULL) {
                        ...
                        $driver->filledRectangle(
                            $component->getLegendBackground(), new awLine($from, $to)
                        );

                        // Draw rectangle border
                        $this->border->rectangle(
                            $driver, $from->move(0, 0), $to->move(0, 0)
                        );
                    }
    ...
    private function drawBase(awDriver $driver, awPoint $p, $width, $height)
    {
        ...
        $this->border->rectangle(
            $driver, $p, $p->move($width, $height)
        );

        $size = $this->border->visible() ? 1 : 0;

        $driver->filledRectangle(
            $this->background, new awLine(
                $p->move($size, $size),
