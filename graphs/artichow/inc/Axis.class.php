...
class awAxis
{
    ...
    protected function getPointFromPosition($position)
    {
        ...
        return $vector->p1->move(
                cos($angle) * $size * $position, -1 * sin($angle) * $size * $position
        );
    }
    ...
    public function autoScale()
    {
        ...
        if ($min == 0) {
            ...
            $this->setRange(
                $min, $intervalReal * $pow
            );
        } else if ($max == 0) {

            $this->setRange(
                $intervalReal * $pow * -1, 0
            );
        }
    ...
    protected function getVector()
    {
        ...
        $vector = new awVector(
                $this->line->p1->move(
                    cos($angle) * $this->padding->left, -1 * sin($angle) * $this->padding->left
                ),
                $this->line->p2->move(
                    -1 * cos($angle) * $this->padding->right, -1 * -1 * sin($angle) * $this->padding->right
                )
        );
