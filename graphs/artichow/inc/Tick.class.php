    ...
    protected function drawTicks(awDriver $driver, awVector $vector, $from, $to)
    {
        ...
        for ($i = $start; round($i, 6) < $stop; $i += $inc) {
            ...
            if ($position % $this->interval === 0) {
                $p = $vector->p1->move(
                    round($i * cos($vector->getAngle()), 6), round($i * sin($vector->getAngle() * -1), 6)
                );
...
//	echo $this->size.':'.$angle.'|<b>'.cos($angle).'</b>/';
        ...
        if ($from !== NULL) {
            $p1 = $p1->move(
                round($this->size * cos($from), 6), round($this->size * sin($from) * -1, 6)
            );
        }

        if ($to !== NULL) {
            $p2     = $p2->move(
                round($this->size * cos($to), 6), round($this->size * sin($to) * -1, 6)
            );
        }
