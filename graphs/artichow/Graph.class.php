...
class awGraph extends awImage
{
        ...
        if (ARTICHOW_CACHE) {
            ...
            if ($this->name !== NULL) {
                ...
                if (is_file($this->fileCache)) {
                    ...
                    $type = self::cleanGraphCache($this->fileCacheTime);

                    if ($type === NULL) {
                        self::deleteFromCache($this->name);
                    } else {
    ...
    public static function cleanCache()
    {
        ...
        if (ARTICHOW_CACHE) {
            ...
            foreach ($glob as $file) {
                ...
                $type = self::cleanGraphCache($file);

                if ($type === NULL) {
                    $name = ereg_replace(".*/(.*)\-time", "\\1", $file);
                    self::deleteFromCache($name);
                }
            }
        }
    }
     ...
     */
    public function draw($mode = self::DRAW_DISPLAY)
    {
        ...
        switch ($mode) {
            ...
            case self::DRAW_DISPLAY :
                $this->sendHeaders();
                echo $data;
                break;

            case self::DRAW_RETURN :
                return $data;
