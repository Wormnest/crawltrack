...
class awPie extends awComponent
{
    ...
    public function __construct($values, $colors = awPie::COLORED)
    {
        ...
        $this->label->setCallbackFunction(function ($value) {
            return $value . '%';
        });
    }
        ...
        foreach ($this->values as $key => $value) {
            ...
            if (array_key_exists($key, $this->explode)) {
                ...
                if ($aliasing) {
                    ...
                    $side->set(
                        max($side->left, $posX * -2), max($side->right, $posX * 2), max($side->top, $posY * -2), max($side->bottom, $posY * 2)
                    );
                } else {
                    $explode = new awPoint(
                            $posX,
...
//			}
            ...
            $image->setSize(
                $width + $side->left + $side->right, $height + $side->top + $side->bottom + $this->size + 1 /* bugs.php.net ! */
            );

            $driver = $image->getDriver(
                $width / $image->width, $height / $image->height, ($width / 2 + $side->left) / $image->width, ($height / 2 + $side->top) / $image->height
            );
        }
        ...
        if ($aliasing) {
            ...
            $mainDriver->copyResizeImage(
                $image, new awPoint($x1 - $side->left / 2, $y1 - $side->top / 2), new awPoint($x1 - $side->left / 2 + $image->width / 2, $y1 - $side->top / 2 + $image->height / 2), new awPoint(0, 0), new awPoint($image->width, $image->height), TRUE
            );
